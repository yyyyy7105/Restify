...
function RevListHost() {
  ...
  useEffect(() => {
    ...
    switch (query.selectedStatus) {
      case "":
        url = `${process.env.REACT_APP_API_URL}:8000/revs/list/${currentUserId}/?user_type=host&page=${query.page}`;
        break;

      default:
        url = `${process.env.REACT_APP_API_URL}:8000/revs/list/${currentUserId}/?status=${query.selectedStatus}&user_type=host&page=${query.page}`;
        break;
    }
